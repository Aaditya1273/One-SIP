...
export async function GET(request: NextRequest) {
  try {
    ...
    // Return platform analytics (mock data for now - can be enhanced with real blockchain queries)
    ...
    analyticsData = {
      ...
      sipMetrics: {
        ...
        popularTokens: [
          { token: "OCT", count: Math.floor(blockchainAnalytics.totalSIPs * 0.486), percentage: 48.6 },
          { token: "USDC", count: Math.floor(blockchainAnalytics.totalSIPs * 0.317), percentage: 31.7 },
          { token: "ETH", count: Math.floor(blockchainAnalytics.totalSIPs * 0.197), percentage: 19.7 }
        ]
      },
      yieldMetrics: {
        totalYieldEarned: blockchainAnalytics.totalYieldGenerated,
        averageAPY: blockchainAnalytics.averageAPY,
        bestPerformingPool: "OneChain-OCT LP",
        bestAPY: 15.8,
        totalPoolsActive: 12,
        yieldDistribution: [
          { pool: "OneChain-OCT LP", yield: blockchainAnalytics.totalYieldGenerated * 0.4, percentage: 40.0 },
          { pool: "OCT Staking", yield: blockchainAnalytics.totalYieldGenerated * 0.3, percentage: 30.0 },
          { pool: "USDC Lending", yield: blockchainAnalytics.totalYieldGenerated * 0.2, percentage: 20.0 },
          { pool: "Others", yield: blockchainAnalytics.totalYieldGenerated * 0.1, percentage: 10.0 }
        ]
      },
      emergencyMetrics: {
